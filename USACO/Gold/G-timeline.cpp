...
// Problem URL: http://www.usaco.org/index.php?page=viewproblem2&cpid=1017
int main() {
    usaco("timeline");
...
class UnweightedGraph {
	...
	bool undirected;
	vector<vector<pair<long long, ll>> > adj; // adjacency neighbor vector
	vector<long long> visited; // visited nodes

    vector<ll> indegree;
    vector<ll> dp; // the current min day satisfying all reqs

	UnweightedGraph(long long nodes, long long edges, bool undirected) {
		this->nodes = nodes;
		this->edges = edges;
		this->undirected = undirected;
		adj = vector<vector<pair<long long, ll>> >(nodes);
    ...
    void toposort() {
        ...
        while(!dq.empty()) {
            ...
            for (pair<ll, long long> neighbor_p : adj[current]) {
                ll neighbor = neighbor_p.f;
                indegree[neighbor]--;

                dp[neighbor] = max(dp[neighbor], dp[current] + neighbor_p.s);
