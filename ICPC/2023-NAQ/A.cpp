...
void solve() {
    ...
    f0r(i, n) {
        ...
        if (m[s].size() < c) {
            ...
            leftover.push_back({i, t});
        }
    }

    vector<pii> all;
    for (auto ind : m) {
        DEBUG(ind.first, ind.second);
        for (auto idx : ind.second) {
            all.push_back(idx);
        }
    }
    DEBUG(leftover);
    DEBUG(k, all.size(), leftover.size());
...
//            cout << all[i].second << endl;
        ...
        f0r(i, k - all.size()) {
            q.push_back(leftover[i]);
//            cout << leftover[i].second << endl;
        }
