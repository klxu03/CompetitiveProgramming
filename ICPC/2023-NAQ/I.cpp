...
void solve() {
    ...
    vector<int> ret;
    // size of the first digit
    for (int sz = 1; sz <= 5 && sz <= s.size(); sz++) {
        int curr_sz = sz;
        bool valid = true;
        int broken = -1;
        ...
        int prev = stoi(builder);
        int beginning = prev;
        int ending = prev;
        int next_ind = sz;

        if (to_string(prev + 1).size() > curr_sz) curr_sz++;

        while (valid && next_ind < s.size()) {
            builder = "";
            for (int i = next_ind; i < next_ind + curr_sz && i < s.size(); i++) {
                builder += s[i];
            }
            int curr = stoi(builder);
            ending = curr;
